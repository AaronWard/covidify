...
df = abstraction.check_specified_country(df, country)
...
daily_cases_df['new_confirmed_cases'] = implementation.get_new_cases(df, 'confirmed')
daily_cases_df['new_deaths'] = implementation.get_new_cases(df, 'deaths')
daily_cases_df['new_recoveries'] = implementation.get_new_cases(df, 'recovered')
...
#Calculate the number of people that are ACTUALLY infected on a given day currently infected = sum of people date - (recovored + died)
#ex: 5 = 10 - (4 - 1)
...
TOP_N_COUNTRIES = implementation.get_top_countries(df)    
...
for country in TOP_N_COUNTRIES:
    ...
    df_list.append(implementation.get_day_counts(tmp_df[tmp_df.country == country], country))
...
class Implementation:
    ...
    def get_new_cases(self, tmp, col):
        "Get difference of sum totals for each date and plot on trendline graph"

    @staticmethod
    @abstractmethod
    def get_day_counts(self, d, country):
        "Get the #of days since 500+ cases confirmed"
...
class ConcreteImplementation(Implementation):
    ...
    # Get the difference of the sum totals for each date and plot them on a trendline graph called past line 63
    def get_new_cases(self, tmp, col):
        diff_list = []
        tmp_df_list = []
        df = tmp.copy()
