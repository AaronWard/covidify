...
############ DAILY CASES ############
# sheets need to be sorted by date value - print('Sorting by datetime...')
...
daily_cases_df['new_confirmed_cases'] = Builder.get_new_cases(df, 'confirmed')
daily_cases_df['new_deaths'] = Builder.get_new_cases(df, 'deaths')
daily_cases_df['new_recoveries'] = Builder.get_new_cases(df, 'recovered')
...
"""
Calculate the number of people that are ACTUALLY infected on a given day -> currently infected = sum of people date - (recovered + died)
ex: 5 = 10 - (4 - 1)
"""
...
TOP_N_COUNTRIES = Builder.get_top_countries(df)    
...
for country in TOP_N_COUNTRIES:
    ...
    df_list.append(Builder.get_day_counts(tmp_df[tmp_df.country == country], country))
    
log_df = pd.concat(df_list, axis=0, ignore_index=True)

#Create and save date of extraction folder
...
class IBuilder(metaclass=ABCMeta):
    def get_similar_countries(self, c, country_list):
        "get all countries in list"

    def check_specified_country(self, df, country):
        "let user filter reports by country, if not found then give a option if the string is similar"
        "Get all unique countries in the data"

    def get_new_cases(self, tmp, col):
        ""

    def get_moving_average(self, tmp, col):
        ""

    def get_exp_moving_average(self, tmp, col):
        ""
    
    def get_top_countries(self, data):
        "Get top N infected countries"

    def get_day_counts(self, d, country):
...
class Builder(IBuilder):
    ...
    def check_specified_country(self, df, country):
        # let user filter reports by country, if not found give a option if the string is similar & Get all unique countries in the data
        self.country_list = list(map(lambda x:x.lower().strip(), set(df.country.values)))
        self.df = check_specified_country(df, country)
    ...
    # Get the difference of the sum totals for each date and plot them on a trendline graph
    def get_new_cases(self, tmp, col):
        diff_list = []
        tmp_df_list = []
        df = tmp.copy()

        for i, day in enumerate(df.sort_values('file_date').file_date.uznique()):
    ...
    def get_day_counts(self, d, country):
        ...
        result_df = data.groupby(['file_date']).agg({'confirmed': 'sum', 'recovered': 'sum', 'deaths': 'sum'})
...
class Product():
    def __init__(self):
        self.c = ""
        self.country_list = []
        self.df = pd.DataFrame(data=None, index=None, columns=None, dtype=None, copy=None)
        self.data = pd.DataFrame(data=None, index=None, columns=None, dtype=None, copy=None)
...
class Director():
    ...
    def construct():
        return Builder()\
        .check_specified_country(df, country)\
        .get_new_cases(df, 'recovered')\
        .get_new_cases(df, 'deaths')\
        .get_new_cases(df, 'confirmed')